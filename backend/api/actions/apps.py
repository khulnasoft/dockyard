...
"""
Checks for current docker id (the one dockyard is running
...
def _update_self(background_tasks):
    ...
    try:
        dockyard = dclient.containers.get(dockyard_id)
    except Exception as exc:
        print(exc)
        if exc.response.status_code == 404:
            raise HTTPException(
                status_code=exc.response.status_code,
                detail="Unable to get Dockyard container ID",
            )
        else:
            status_code = 500
            detail = exc.args[0]
            raise HTTPException(status_code=status_code, detail=detail)
    background_tasks.add_task(update_self_in_background, dockyard)
    return {"result": "successful"}


"""
Spins up a watchtower instance with --cleanup and 
--run-once pointed at the current ID of dockyard.
"""


def update_self_in_background(dockyard):
    dclient = docker.from_env()
    volumes = {"/var/run/docker.sock": {"bind": "/var/run/docker.sock", "mode": "rw"}}
    print("**** Updating " + dockyard.name + "****")
    dclient.containers.run(
        image="containrrr/watchtower:latest",
        command="--cleanup --run-once " + dockyard.name,
        remove=True,
        detach=True,
        volumes=volumes,
    )
...
def check_self_update():
    ...
    return _check_updates(dockyard.image.tags[0])
